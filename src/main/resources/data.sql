...
WHERE NOT EXISTS (SELECT 1 FROM sales_people.message_template WHERE mst_type = 'LOGIN_OTP');
...
WHERE NOT EXISTS (SELECT 1 FROM sales_people.message_template WHERE mst_type = 'PASSWORD_RESET');
